...
const Dashboard = () => {
	...
	const getDashboard = async () => {
		...
		// 	console.log("response data",response.data.data.menu_details.length);
		// 	// if(response.data.data.menu_details.length===0){
		// 	// 	localStorage.clear();
		...
		//*********Added on 23.08.2023 to fix RED screen issue*/
		await DashboardService.getDashboardDetails(userProfile).then((response) => {
					console.log("response data",response.data.data.menu_details.length);
			if(response.data.data.menu_details.length===0){
				localStorage.clear();
